...
def verify_manifest_index(root: Path, manifest_path: Path) -> None:
    ...
    for entry_lines in entries:
        ...
        if len(record_lines) == 1 and scalar(record_lines[0]) == EXPECTED_RECORD_ID:
            matches.append(entry_lines)
    if len(matches) != 1:
        raise StudyGuardError(
            "canonical study index must contain exactly one matching record entry"
        )
    entry_lines = matches[0]
    ...
    version = field("record_version", r"^\s+record_version:\s*")
    artifact_path = field("artifact_path", r"^\s+artifact_path:\s*")
    indexed_digest = field("artifact_digest.value", r"^\s+value:\s*[\"']?[a-f0-9]{64}[\"']?\s*$")
    status = field("registry_status", r"^\s+registry_status:\s*")
    if version != "1":
