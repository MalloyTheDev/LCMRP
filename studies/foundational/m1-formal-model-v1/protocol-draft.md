# Foundational Study Protocol Draft: Bounded Formal Analysis of FMO-0.1

> **DRAFT — NOT FROZEN — DO NOT EXECUTE.** This document prepares a Layer 1 `FORMAL_ANALYSIS` study for freeze review. It is not an authoritative registration, result, finding, closeout, proof, validation, evidence award, or implementation authorization. No planned analyzer output may be created or inspected from this draft.
...
| Reviewers | `LCMRP-M1-INDEPENDENT-PROTOCOL-REVIEW-LANE`; freeze authority assignment pending |
| Freeze timestamp and authority | Not frozen; no authority asserted |
| Immutable freeze artifact | None while draft |
| Repository revision | Pending exact freeze commit |
| Supersession lineage | None; initial record version |

The subject resolves through the active foundational-subject registry. That registry fixes identity and provenance only. This draft neither adopts FMO-0.1 nor activates a study.
...
| `SOURCE-FSC-01` | The accepted Layer 1 formal-analysis profile requires a versioned formal system, assumptions, propositions, satisfiability, entailment, non-entailment, countermodel, provenance, semantic-validity, and counterexample-search obligations. | `docs/program/FOUNDATIONAL_STUDY_CONTRACT.md`; repository revision to be frozen | `PRIOR_WORK` | Inspected; exact freeze digest pending | A contract mismatch invalidates the study design and requires governance review. |
| `SOURCE-KERNEL-01` | The study-local finite Boolean modules, variables, constraints, bounds, and exclusions define the only machine-checkable semantics used by this study. | `artifacts/formal-system.json`, version 1; exact digest to be bound | `FORMAL_INPUT` | Syntax checked only while draft; no semantic result accessed | Any substitution, parse ambiguity, or incomplete freeze binding halts execution. |
...
1. Independent reviewers inspect this draft, the draft manifest, exact subject, contract, profile definition, bounded formal system, assumptions, propositions, configuration, environment, tool provenance, and analyzer source without executing the analyzer.
2. The freeze authority resolves all review findings before result access, creates a separate final protocol and authority-bound freeze attestation, binds final raw-byte digests, and publishes a schema-valid `FROZEN` record through the canonical record path. This draft does not perform that authority action.
...
- **Formal-system artifact:** `studies/foundational/m1-formal-model-v1/artifacts/formal-system.json`, artifact `LCMRP-ARTIFACT-0002-M1-FMO-BOUNDED-FORMAL-SYSTEM` version 1; exact freeze digest pending review.
...
| `SOURCE-KERNEL-01` | `FORMAL_SPECIFICATION` | `FORMAL_INPUT` | Artifact version 1; digest to be verified at freeze | All and only declared kernel modules | Public repository; repository license applies | No human subjects or participant data |
| `SOURCE-ASSUMPTIONS-01` | `OTHER_NON_HUMAN_SOURCE` | `DEVELOPMENT` | Artifact version 1; digest to be verified at freeze | All FMO-A01 through FMO-A12 rows | Public repository; repository license applies | No human subjects or participant data |
| `SOURCE-PROPOSITIONS-01` | `OTHER_NON_HUMAN_SOURCE` | `DEVELOPMENT` | Artifact version 1; digest to be verified at freeze | Full catalog, queries, invariant targets, and named witnesses | Public repository; repository license applies | No human subjects or participant data |
...
- **Source revision and dirty-state policy:** Freeze an exact commit. Execution requires that exact revision, no uncommitted change to any bound artifact, and a separately recorded status of unrelated files. A change to a bound byte requires a new digest and, after freeze, a superseding record.
...
- **Profile-definition artifact:** `artifacts/method-profile-definition.json`, version 1; raw-byte SHA-256 to be recorded at freeze.
...
- **Protocol and freeze artifacts:** This draft has no freeze effect. A separately reviewed final protocol, final manifest, canonical record, and authority-bound attestation must be immutable and mutually digest-bound before execution.
...
- **Foundational finding status:** No finding. This is an unfrozen draft with no results.
- **Mechanism evidence labels:** Not applicable.
- **Independent validation:** Not attempted. Independent protocol review is required before freeze; independent replication is required after any findings before broader reliance.
- **Missing validation obligations:** Authoritative freeze; exact final artifact digests; analyzer code review; schema-valid canonical record; clean-revision attestation; execution; atomic dispositions for seven analyses; semantic adjudication; immutable closeout; independent replication; and fuller formalization of the open FMO obligations.
...
- **Next falsifiable question:** After this protocol is independently reviewed and authoritatively frozen, do the exact bounded kernel and semantic mappings survive the seven preregistered analyses without a retained contradiction, counterexample, missing witness, authority/deletion boundary failure, or invalid encoding?
...
- **Independent review or replication required:** One independent protocol and encoding review before freeze; two independent semantic mappings during analysis; and an independently implemented replay before any claim is treated as independently validated.
