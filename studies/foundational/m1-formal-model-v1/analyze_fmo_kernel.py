...
def verify_manifest_index(root: Path, manifest_path: Path) -> None:
    ...
    entry_pattern = re.compile(
        r"(?ms)^  - record_id:\s*[\"']?"
        + re.escape(EXPECTED_RECORD_ID)
        + r"[\"']?\s*$"
        + r"(?P<body>.*?)(?=^  - record_id:|\Z)"
    )
    ...
    def field(pattern: str, label: str) -> str:
        found = re.search(pattern, body, flags=re.MULTILINE)
        if found is None:
            raise StudyGuardError(f"canonical index entry lacks {label}")
        return found.group(1)

    version = field(r"^    record_version:\s*([0-9]+)\s*$", "record_version")
    artifact_path = field(
        r"^    artifact_path:\s*[\"']?([^\"'\s]+)[\"']?\s*$",
        "artifact_path",
    )
    indexed_digest = field(
        r"^        value:\s*[\"']?([a-f0-9]{64})[\"']?\s*$",
        "artifact_digest.value",
    )
    status = field(
        r"^    registry_status:\s*[\"']?([A-Z]+)[\"']?\s*$",
        "registry_status",
    )
    if version != "1":
