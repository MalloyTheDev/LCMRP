...
[`foundational-record-index.schema.json`](../schemas/foundational-record-index.schema.json) defines the append-oriented indexes in `foundational-studies.yaml`, `research-findings.yaml`, and `foundational-study-closeouts.yaml`. The foundational-subject registry contains exactly two `ACTIVE` M1 candidate identities: the exact candidate taxonomy and formal-model bytes. The foundational-study registry contains exactly two `ACTIVE`, `FROZEN` Layer 1 preregistrations bound to those subjects. The research-finding and foundational-closeout registries remain empty, as do the mechanism, experiment, and evidence registries.

Subject registration makes exact candidate bytes resolvable. Study registration freezes questions, methods, planned analyses, inputs, and output locations before result access. Neither action adopts or validates a subject, reports a result, closes a study, or awards evidence.
